...
const MediMapUtils = {
    ...
    // Medicine Search Utilities
    searchMedicineNames(query) {
        // Return empty array for empty queries
        if (!query || query.trim().length === 0) return [];
        
        // Get results from medicine-names.js searchMedicineName function
        const searchResults = searchMedicineName(query);
        
        // Format the results based on current language
        const currentLang = localStorage.getItem('language') || 'bn';
        return searchResults.map(result => ({
            displayName: currentLang === 'bn' ? result.bn : result.en,
            altName: currentLang === 'bn' ? result.en : result.bn,
            value: result.en // Use English name as value for consistency
        }));
    },
...
// Add symptom selection styles
function addSymptomStyles() {
    if (!document.querySelector('#medimap-symptom-styles')) {
        const symptomStyles = document.createElement('style');
        symptomStyles.id = 'medimap-symptom-styles';
        symptomStyles.textContent = `
            .symptom-card {
                cursor: pointer;
        ...
        `;
        document.head.appendChild(symptomStyles);
    }
}
...
function initializeSearch() {
    ...
    symptomCards.forEach(card => {
        card.addEventListener('click', () => {
            toggleSymptom(card.dataset.symptom, card);
        });
    });
...
// Switch between medicine and symptom tabs
...
function toggleSymptom(symptom, cardElement) {
    ...
    updateSymptomSearchButton();
}

// Update symptom search button text
function updateSymptomSearchButton() {
    const searchBtn = document.querySelector('#symptom-search .search-btn');
    if (searchBtn) {
        if (selectedSymptoms.length > 0) {
            searchBtn.textContent = `নির্বাচিত ${selectedSymptoms.length}টি লক্ষণের ওষুধ খুঁজুন`;
        } else {
            searchBtn.innerHTML = '<i class="fas fa-search"></i> নির্বাচিত লক্ষণ অনুযায়ী ওষুধ খুঁজুন';
        }
    }
}
...
function searchMedicine() {
    ...
    if (!query) {
        UIUtils.showNotification('ওষুধের নাম লিখুন', 'warning');
        return;
    }
...
function searchBySymptom() {
    if (selectedSymptoms.length === 0) {
        UIUtils.showNotification('অন্তত একটি লক্ষণ নির্বাচন করুন', 'warning');
        return;
    }

    // Store search data
    localStorage.setItem('searchType', 'symptom');
    localStorage.setItem('searchQuery', JSON.stringify(selectedSymptoms));
...
function searchEmergency() {
    // Store search data
    localStorage.setItem('searchType', 'emergency');
    localStorage.setItem('searchQuery', 'emergency');

    // Redirect to results page
    window.location.href = 'search-results.html';
}

// Export search functions globally
window.searchMedicine = searchMedicine;
window.searchBySymptom = searchBySymptom;
window.searchEmergency = searchEmergency;
