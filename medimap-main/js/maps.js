...
// Maps Configuration
const MAPS_CONFIG = {
    API_KEY: 'AIzaSyCV4yFyJgyoLZfkFwPMH-KPXqRVQukWbDQ',
    DEFAULT_CENTER: {
        lat: 23.8223, // Corrected Mirpur-1 coordinates
        lng: 90.3654
    },
    DEFAULT_ZOOM: 15,
    SEARCH_RADIUS: 2000, // 2km in meters (default)
    RADIUS_OPTIONS: [
...
let currentRadius = MAPS_CONFIG.SEARCH_RADIUS;
...
const MapsManager = {
    ...
    // Filter pharmacies by location and radius
    filterPharmaciesByLocation(pharmacies, userLat, userLng, radiusMeters = currentRadius) {
        ...
        return pharmacies.filter(pharmacy => {
            ...
            pharmacy.distanceFromUser = distance; // Store distance in km
    ...
    addPharmacyMarkers(pharmacies) {
        ...
        if (userLocation) {
            filteredPharmacies = this.filterPharmaciesByLocation(
                pharmacies,
                userLocation.lat,
                userLocation.lng,
                currentRadius
    ...
    async getCurrentLocation() {
        ...
        return new Promise((resolve, reject) => {
            if (!navigator.geolocation) {
                const error = new Error('আপনার ব্রাউজার জিওলোকেশন সাপোর্ট করে না');
                if (locationText) {
                    locationText.innerHTML = '<i class="fas fa-exclamation-triangle"></i> জিওলোকেশন সাপোর্ট করে না';
                }
                reject(error);
                return;
            }
            ...
            navigator.geolocation.getCurrentPosition(
                (position) => {
                    ...
                    window.userLocation = userPosition;

                    // Update location display
                    this.updateLocationDisplay(userPosition.lat, userPosition.lng);
                    ...
                    // Trigger search results update if available
                ...
                (error) => {
                    ...
                    switch(error.code) {
                        ...
                        case error.POSITION_UNAVAILABLE:
                            errorMessage = 'অবস্থান উপলব্ধ নেই';
                            break;
                        case error.TIMEOUT:
                            errorMessage = 'অবস্থান খুঁজতে বেশি সময় লাগছে';
                            break;
                    }
    ...
    updateLocationDisplay(lat, lng) {
        const locationText = document.getElementById('locationText');
        if (locationText) {
            locationText.innerHTML = `<i class="fas fa-map-marker-alt"></i> ঠিকানা খুঁজছি...`;
            this.reverseGeocode(lat, lng)
                .then((result) => {
                    locationText.innerHTML = `<i class="fas fa-map-marker-alt"></i> ${result.formatted_address}`;
                })
                .catch(() => {
                    locationText.innerHTML = `<i class="fas fa-map-marker-alt"></i> Lat: ${lat.toFixed(4)}, Lng: ${lng.toFixed(4)}`;
                });
        }
    },
